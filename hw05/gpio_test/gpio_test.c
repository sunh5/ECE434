...
static unsigned int gpioLED2 = 60;      ///< hard coding the LED gpio for this example to P9_12GPIO60)
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   result = request_irq(irqNumber,             // The interrupt number requested
                        ...
                        IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,   // Interrupt on both edges (button press, not release)
                        "ebb_gpio_handler",    // Used in /proc/interrupts to identify the owner
                        NULL);                 // The *dev_id for shared interrupt lines, NULL is okay
                        
   result2 = request_irq(irqNumber2,             
